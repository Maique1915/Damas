...
let player1Wins = 0;
let player2Wins = 0;
let player1Captured = 0; // Peças do Jogador 2 capturadas pelo Jogador 1
let player2Captured = 0; // Peças do Jogador 1 capturadas pelo Jogador 2
...
function updateCaptureCountDisplay() {
    const p1CapturedCountElement = document.getElementById('player1-captured-count');
    const p2CapturedCountElement = document.getElementById('player2-captured-count');

    if (p1CapturedCountElement) {
        p1CapturedCountElement.textContent = player1Captured;
    }
    if (p2CapturedCountElement) {
        p2CapturedCountElement.textContent = player2Captured;
    }
}
...
function makeMove(pieceInfo, move) {
    ...
    // Atualização lógica do tabuleiro
    ...
    if (captures) {
        ...
        if (capturedPieceElement) {
            // *** INÍCIO DA LÓGICA DE CONTAGEM ***
            // Determina quem capturou (o jogador atual)
            if (currentPlayer === PLAYER1) {
                player1Captured++; // Jogador 1 capturou uma peça do Jogador 2
            } else {
                player2Captured++; // Jogador 2 capturou uma peça do Jogador 1
            }
            console.log(`Peças capturadas: P1=${player1Captured}, P2=${player2Captured}`);
            // Atualiza a exibição na tela
            updateCaptureCountDisplay();
             // *** FIM DA LÓGICA DE CONTAGEM ***

            boardState[captures.row][captures.col] = null; // Atualiza estado lógico
            capturedPieceElement.remove(); // Remove do DOM
            captured = true; // Marca que houve captura para lógica subsequente
        }
    }
...
function resetGame() {
    ...
    // --- ZERA A CONTAGEM DE CAPTURAS PARA O NOVO JOGO ---
    player1Captured = 0;
    player2Captured = 0;
    // --- FIM DO RESET ---
    ...
    // Atualiza a exibição dos placares (vitórias e capturas zeradas)
    updateScoreDisplay();
    updateCaptureCountDisplay(); // << Chama a nova função aqui
}
...
// Exibe os placares iniciais (0-0 e 0 capturas)
updateScoreDisplay();
updateCaptureCountDisplay(); // << Chama a nova função aqui
